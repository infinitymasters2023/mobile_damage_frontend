...
export default function LoginComponent() {
  ...
  const [form, setForm] = useState<UserType>({ name: "", password: "" }); // Using 'name' for username
    ...
    >
      ...
      <div className="flex flex-col items-center text-center mb-8">
         <div className="inline-flex items-center gap-3 p-3 rounded-2xl bg-white border border-blue-500/20 mb-4">
          <Image src="/img/infyeazy_logo.svg" alt="logo" width={120} height={40} className="object-contain" priority />
        </div>
      ...
      <form onSubmit={handleLogin} className="space-y-4">
        ...
        <div className="relative group">
          <Lock className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-600 group-focus-within:text-[#0066FF] transition-colors" size={18} />
          <input
          ...
          />
          <button type="button" onClick={() => setShow(!show)} className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-600 hover:text-white transition-colors">
      ...
      <p className="mt-8 text-center text-[10px] font-bold text-slate-600 uppercase tracking-widest">
        New Node? <Link href="/signup" className="text-[#0066FF] hover:text-blue-400 transition-colors ml-1 underline underline-offset-4">SIGN UP</Link>
      </p>
    </motion.div>
  );
}
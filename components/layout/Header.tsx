...
// Added Menu and X icons
import { LogOut, User, ChevronDown, Settings, Menu, X } from "lucide-react";

interface HeaderProps {
  title: string;
  // If you are using a global state or parent state for the sidebar:
  isSidebarOpen?: boolean;
  setSidebarOpen?: (open: boolean) => void;
}

export default function Header({ title, isSidebarOpen, setSidebarOpen }: HeaderProps) {
  ...
  const userAvatar = session?.user?.image || "https://api.dicebear.com/7.x/avataaars/svg?seed=Guest";
  ...
  return (
    <header className="sticky top-0 z-50 w-full h-16 border-b border-white/[0.08] bg-[#0d0d0d] flex items-center justify-between px-4 lg:px-10 shrink-0">
      
      {/* LEFT: Hamburger + Logo */}
      <div className="flex items-center gap-4">
        {/* Hamburger Menu - Visible only on Mobile/Tablet (lg:hidden) */}
        <button 
          onClick={() => setSidebarOpen?.(!isSidebarOpen)}
          className="p-2 -ml-2 text-white/70 hover:bg-white/5 rounded-lg lg:hidden"
        ...
        <Link href="/" className="block group">
          <div className="bg-white px-3 py-1 rounded-md shadow-lg group-hover:bg-slate-100 transition-colors">
            <Image
              src={Logo}
              alt="InfyEazy"
              width={90}
        ...
        <h1 className="text-[10px] font-bold uppercase tracking-[0.2em] text-white/60 hidden lg:block">
          {title}
        </h1>
      </div>
      ...
      <div className="relative" ref={dropdownRef}>
        ...
        {/* Dropdown Menu */}
        {isOpen && (
          <div className="absolute right-0 mt-3 w-44 bg-[#121212] border border-white/10 rounded-xl shadow-[0_10px_40px_rgba(0,0,0,0.7)] py-1.5 z-[60]">
